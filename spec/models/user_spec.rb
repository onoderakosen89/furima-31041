  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録ができない時' do
      ...
      it 'passwordに英小文字が含まれない場合無効な状態であること' do
        @user.password = '1' + 'A' * 6
        @user.password_confirmation = '1' + 'A' * 6
        @user.valid?
        expect(@user.errors.full_messages).to include('Password は半角6~12文字英大文字・小文字・数字それぞれ１文字以上含む必要があります')
      end
      ...
      it 'passwordに数字が含まれない場合無効な状態であること' do
        @user.password = 'A' + 'a' * 6
        @user.password_confirmation = 'A' + 'a' * 6
        @user.valid?
        expect(@user.errors.full_messages).to include('Password は半角6~12文字英大文字・小文字・数字それぞれ１文字以上含む必要があります')
      end

      it 'passwordが5文字以下なら無効な状態であること' do
        @user.password = '1A' + 'a' * 3
        @user.password_confirmation = '1A' + 'a' * 3
        @user.valid?
        expect(@user.errors.full_messages).to include('Password は半角6~12文字英大文字・小文字・数字それぞれ１文字以上含む必要があります')
      end

      it 'passwordが13文字以上なら無効な状態であること' do
        @user.password = '1A' + 'a' * 14
        @user.password_confirmation = '1A' + 'a' * 14
        @user.valid?
        expect(@user.errors.full_messages).to include('Password は半角6~12文字英大文字・小文字・数字それぞれ１文字以上含む必要があります')
      end
